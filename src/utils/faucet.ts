...
/**
 * Format error messages from mint operations into user-friendly text.
 * Parses common error patterns and returns readable messages.
 */
export const formatErrorMessage = (error: Error | unknown): string => {
    ...
    if (errorMessage.includes('An internal error was received')) {
        ...
        const detailsMatch = errorMessage.match(/Details: ([^V]+)/);
        if (detailsMatch) {
            const details = detailsMatch[1].trim();
            return formatErrorMessage(new Error(details));
        }
    ...
    return cleanMessage || 'An error occurred while minting tokens';
};
