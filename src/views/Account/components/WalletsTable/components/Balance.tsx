...
const BalanceInfo: React.FC<ItemProps> = ({index, currencies, balance, value}) => {
        ...
        <div>
            <StyledBalance>
                <StyledWalletInfoContainer>
                    <StyledBalanceText
                    >{ balance != null ? ordinaryFormat(balance) : 0 } {shortName}</StyledBalanceText>
                    <StyledBalanceSubtitle
                    >{ value != null ? usdFormat(value) : 0 }</StyledBalanceSubtitle>
                </StyledWalletInfoContainer>
            </StyledBalance>
        </div>
    )
...
const RenderBalance: React.FC<BalanceContainerProps> = ({balance, value, index}) => {
    return (
        <BalanceInfo 
            balance={balance}
            value={value}
