    ...
    return (
        <StyledRateList>
            <StyledCurrencySelector>
                ...
                <StyledMainCurrency style={{ background: '#172734' }}>
                    ETH
                </StyledMainCurrency>
            </StyledCurrencySelector>
            <StyledRateTable>
                <StyledRateTableHeaderItem position={'left'}>
                    TOKEN
                </StyledRateTableHeaderItem>
                <StyledRateTableHeaderItem position={'center'}>
                    LEND
                </StyledRateTableHeaderItem>
                <StyledRateTableHeaderItem position={'center'}>
                    BORROW
                ...
                {testRates.map((rate, i) => {
                    return (
                        <React.Fragment key={i}>
                            ...
                            <StyledRateTableItem
                                position={'center'}
                                color={theme.colors.ratesBlue}
                            >
                                {rate.lend.toFixed(2)}%
                            </StyledRateTableItem>
                            <StyledRateTableItem
                                position={'center'}
                                color={theme.colors.ratesRed}
                            >
                                {rate.borrow.toFixed(2)}%
                            </StyledRateTableItem>
                        </React.Fragment>
                    );
                })}
