...
import { HDWallet, PKWallet, TestNetPath, useNewFilWalletProvider } from '../../../services/filecoin'
...
const RenderPrivateKeyContainer: React.FC<ContainerProps> = ({ onDismiss, selectedTab }) => {
    ...
    const handleChangeMnemonic = useCallback((data: string) => {
		setMnemonic(data)
		filAddressFromMnemonic(data)
	},[setMnemonic, setMnemonicAddr])

	const handleChangePrivateKey = useCallback((data: string) => {
		setPrivateKey(data)
		filAddressFromPrivateKey(data)
	}, [setPrivateKey, setPrivateKeyAddr])

	const filAddressFromMnemonic = useCallback((data: string) => {
	...
	const filAddressFromPrivateKey = useCallback((data: string) => {
    ...
    const handleCreateFilPKWallet = useCallback(async () => {
        try {
			if (filProviders && privateKey != '' && walletProvider == null) {
				const provider = await filProviders.PrivateKeyProvider(privateKey)
	...
	return (
		<Modal>
			...
			<ModalContent paddingBottom={'0'} paddingTop={'0'}>
				<StyledModalSubtitle>Enter your mnemonic phrase or private key to import your account.</StyledModalSubtitle>
				<StyledTabButtons>
					{ordersTabs.map((tab, i) => (
						<Button
							...
							style={{
								...
								borderWidth: 1,
								borderColor: selectedTab === tab ? theme.colors.darkBlue : theme.colors.background,
								borderBottom: selectedTab === tab ? theme.colors.darkBlue : theme.colors.background,
