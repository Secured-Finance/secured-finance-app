...
/*
TO-DO: add prettier for auto-formatting
*/
...
const useLoanHistory = () => {
  ...
  const fetchLoanHistory = useCallback(async (effectState) => {
    dispatch(startSetHistory());
    const history: Promise<LoansHistory> = await getLoansHistory(
      loanContract,
      account
    );
  ...
  useEffect(() => {
    /*
    Why isMounted is here?
    we need to check if the component is mounted after async operations before updating the state
    */
    // hotfix
    let effectState = { isActive: true }
    if (securedFinance && loanContract && account) {
      fetchLoanHistory(effectState);
    }
    return () => {
...
export const useBorrowHistory = () => {
  ...
  const fetchBorrowHistory = useCallback(async () => {
    dispatch(startSetHistory());
    const history: Promise<LoansHistory> = await getBorrowHistory(
      loanContract,
      account
    );
    let parsedHistory: Array<HistoryTableData> = [];
