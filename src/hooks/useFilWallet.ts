...
export const useFilecoinWalletStore = (): WalletBase => {
    ...
    const updateFilStore = useCallback(
        async (
            wasmLoaded: boolean,
            walletProvider: Filecoin,
            filUSDPrice: number,
            filWalletAddress: string
        ) => {
            if (filWalletAddress && (!wasmLoaded || walletProvider === null)) {
                dispatch(updateFilWalletViaRPC(filWalletAddress));
                return;
            }
    ...
    useEffect(() => {
        ...
        updateFilStore(loaded, walletProvider, price, filAddr);
    }, [
        price,
        change,
        dispatch,
