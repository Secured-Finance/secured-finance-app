...
import { fetchWallet, fetchWalletFailure, updateFilWalletActions, updateFilWalletAddress, updateFilWalletAssetPrice, updateFilWalletBalance, updateFilWalletDailyChange, updateFilWalletPortfolioShare, updateFilWalletUSDBalance } from '../store/wallets'
...
export const useFilecoinAddress = () => {
    ...
    const filecoinAddr = useSelector((state: RootState) => state.wallets.filecoin.address)
    const walletProvider = useSelector((state: RootState) => state.filWalletProvider.walletProvider)

    const fetchFilStore = useCallback(async (isMounted: boolean) => {
        await dispatch(fetchWallet())
        if (loaded && walletProvider != null) {
            const [filAddr] = await walletProvider.wallet.getAccounts(0, 1, Network.TEST)
...
export const useFilecoinBalance = () => {
    const dispatch = useDispatch()
    const { loaded } = useFilWasm()
    const filecoinBalance = useSelector((state: RootState) => state.wallets.filecoin.balance)
    const walletProvider = useSelector((state: RootState) => state.filWalletProvider.walletProvider)
...
export const useFilecoinUSDBalance = async () => {
    const dispatch = useDispatch()
    const { loaded } = useFilWasm()
    const filecoinUSDBalance = useSelector((state: RootState) => state.wallets.filecoin.usdBalance)
    const filUSDPrice = useSelector((state: RootState) => state.assetPrices.filecoin.price)
    const walletProvider = useSelector((state: RootState) => state.filWalletProvider.walletProvider)
    ...
    const fetchFilStore = useCallback(async (isMounted: boolean) => {
        ...
        if (loaded && walletProvider != null && filUSDPrice != 0) {
            const [filAddr] = await walletProvider.wallet.getAccounts(0, 1, Network.TEST)
            const balance = await walletProvider.getBalance(filAddr)
            let usdBalance = new BigNumber(balance.toFil()).times(new BigNumber(filUSDPrice)).toNumber()
...
export const useFilecoinWalletStore = () => {
    ...
    const fetchFilStore = useCallback(async (isMounted: boolean) => {
        ...
        const balance = await walletProvider.getBalance(filAddr)
        dispatch(updateFilWalletBalance(balance.toNumber()))
        let usdBalance = new BigNumber(balance.toFil()).times(new BigNumber(price)).toNumber()
        dispatch(updateFilWalletUSDBalance(usdBalance))
        dispatch(updateFilWalletAssetPrice(price))
        dispatch(updateFilWalletDailyChange(change))
        dispatch(updateFilWalletActions(actObj))
        let portfolioShare = new BigNumber(usdBalance).times(100).dividedBy(new BigNumber(totalUSDBalance)).toNumber()
        dispatch(updateFilWalletPortfolioShare(portfolioShare))
	}, [dispatch, price, totalUSDBalance, loaded, walletProvider, change])
    
	useEffect(() => {
        let isMounted = true;
        if ( loaded && totalUSDBalance != 0 && walletProvider != null && price != 0 && change != 0) {
            fetchFilStore(isMounted)
        }
        return () => { isMounted = false };
